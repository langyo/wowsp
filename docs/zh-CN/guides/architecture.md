...
```mermaid
flowchart TD
    A["游戏启动"] --> B["mod 启动器拉起 WoWSP 透明覆盖窗口"]
    B --> C["Rust：轮询 &lt;game&gt;/replays/tempArenaInfo.json 获取实时阵容（commands/arena_info.rs）"]
    C --> D["用户按住 Tab"]
    D --> E["Rust：截取游戏窗口 + 检测中央名单区域（commands/overlay.rs）"]
    E --> F["webui：将渲染的阵容重新锚定到检测到的矩形（features/overlay）"]
    F --> G["用户松开 Tab → 覆盖层隐藏"]
```
