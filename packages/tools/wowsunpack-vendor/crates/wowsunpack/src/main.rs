...
enum Commands {
    ...
    /// Export ship armor collision meshes as a colored GLB file
    ExportArmorGlb {
        /// Ship name (model directory prefix, e.g. "PJSB018")
        name: String,

        /// Hull upgrade (e.g. "A", "B")
        #[arg(long)]
        hull: Option<String>,
...
fn run() -> Result<(), Report> {
    ...
    match args.command {
        ...
        Commands::ExportArmorGlb { name, hull, output } => {
            let Some(vfs) = &vfs else {
                bail!("VFS required. Use --game-dir to specify a game install.");
            };

            run_export_armor_glb(vfs, &name, &game_dir, game_version, hull.as_deref(), &output)?;
        }
...
fn run_export_armor_glb(
    ...
    let options = ShipExportOptions { hull: hull_selection.map(|s| s.to_string()), textures: false, ..Default::default() };
    let ctx = assets.load_ship(name, &options).context("Failed to load ship")?;
    let meshes = ctx.interactive_armor_meshes().context("Failed to build armor meshes")?;

    if meshes.is_empty() {
        bail!("No armor meshes found for ship {}", name);
    }
    ...
    for mesh in &meshes {
        ...
        for (vi, pos) in mesh.positions.iter().enumerate() {
            let [x, y, z] = if let Some(ref m) = mesh.transform {
                let [m00,m01,m02,m03,m10,m11,m12,m13,m20,m21,m22,m23,_,_,_,_] = *m;
                [m00*pos[0]+m01*pos[1]+m02*pos[2]+m03,
                 m10*pos[0]+m11*pos[1]+m12*pos[2]+m13,
                 m20*pos[0]+m21*pos[1]+m22*pos[2]+m23]
            } else { *pos };
            positions.extend_from_slice(&[x, y, z]);
            let c = mesh.colors[vi];
            colors.extend_from_slice(&[c[0], c[1], c[2], c[3]]);
        }
        for idx in &mesh.indices { indices.push(idx + base); }
    }

    normalize_to_200(&mut positions);

    write_armor_glb(output, &positions, &colors, &indices)?;
    eprintln!("Exported {} armor tris to {}", indices.len() / 3, output.display());
    Ok(())
}

/// Scale + centre positions so the bounding box fits within 200 units, centred
/// at the origin.  Matches the visual model baker's normalisation.
fn normalize_to_200(positions: &mut [f32]) {
...
fn write_armor_glb(path: &Path, positions: &[f32], colors: &[f32], indices: &[u32]) -> Result<(), Report> {
    use std::io::Write;
    ...
    let mut bin = Vec::new();
    let po = bin.len(); bin.extend(&pos_bytes);
    let co = bin.len(); bin.extend(&col_bytes);
    let io = bin.len(); bin.extend(&idx_bytes);

    let json = serde_json::json!({
        "asset":{"version":"2.0","generator":"wowsunpack"},
        ...
        "bufferViews":[
            {"buffer":0,"byteOffset":po,"byteLength":pos_bytes.len(),"target":34962},
            {"buffer":0,"byteOffset":co,"byteLength":col_bytes.len(),"target":34962},
            {"buffer":0,"byteOffset":io,"byteLength":idx_bytes.len(),"target":34963},
        ],
        "accessors":[
            {"bufferView":0,"componentType":5126,"count":n_verts,"type":"VEC3"},
            {"bufferView":1,"componentType":5126,"count":n_verts,"type":"VEC4"},
            {"bufferView":2,"componentType":idx_type,"count":indices.len(),"type":"SCALAR"},
        ],
    });
    ...
    f.write_all(&0x46546C67u32.to_le_bytes())?;
    f.write_all(&2u32.to_le_bytes())?;
    f.write_all(&(total as u32).to_le_bytes())?;
    f.write_all(&(json_bytes.len() as u32).to_le_bytes())?;
    f.write_all(&0x4E4F534Au32.to_le_bytes())?;
    f.write_all(&json_bytes)?;
    f.write_all(&(bin.len() as u32).to_le_bytes())?;
    f.write_all(&0x004E4942u32.to_le_bytes())?;
    f.write_all(&bin)?;
    Ok(())
}
