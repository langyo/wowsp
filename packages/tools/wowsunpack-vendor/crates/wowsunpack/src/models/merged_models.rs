...
pub fn parse_merged_models(file_data: &[u8]) -> Result<MergedModels, Report<MergedModelsError>> {
    ...
    for i in 0..models_count {
        ...
        match parse_model_record(file_data, rec_base) {
            Ok(record) => models.push(record),
            Err(e) => {
                eprintln!("Warning: skipping model[{i}]: {e}");
                skipped += 1;
            }
        }
    }
    if skipped > 0 {
        eprintln!("  {skipped}/{models_count} model records skipped due to parse errors");
    }
    ...
    let mut skel_skipped = 0usize;
    for i in 0..skeletons_count {
        let skel_base = skeletons_offset + i * SKELETON_SIZE;
        match parse_skeleton_proto(file_data, skel_base) {
            Ok(skeleton) => skeletons.push(skeleton),
            Err(e) => {
                eprintln!("Warning: skipping skeleton[{i}]: {e}");
                skel_skipped += 1;
            }
        }
    }
    if skel_skipped > 0 {
        eprintln!("  {skel_skipped}/{skeletons_count} skeleton records skipped due to parse errors");
    }
