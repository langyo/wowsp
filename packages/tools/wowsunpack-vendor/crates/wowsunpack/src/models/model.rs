...
fn parse_model_at(blob_data: &[u8], base: usize) -> Result<ModelPrototype, Report<ModelError>> {
    ...
    let skel_ext_res_ids = if skel_ext_count > 0 {
        ...
        if abs + need > blob_data.len() {
            ...
            let ids: Vec<u64> = repeat(skel_ext_count, le_u64).parse_next(input)
                .map_err(|_: ErrMode<ContextError>| ModelError::DataTooShort { offset: abs, need, have: blob_data.len() })?;
            ids
        }
    } else {
        Vec::new()
    ...
    let animations = if animations_count > 0 {
        ...
        if abs + need > blob_data.len() {
            ...
            for i in 0..animations_count {
                match parse_model_at(blob_data, abs + i * MODEL_ITEM_SIZE) {
                    Ok(a) => anims.push(a),
                    Err(_) => {} // skip broken animation entries
                }
            }
            anims
        }
    ...
    let dyes = if dyes_count > 0 {
        ...
        parse_dye_entries(blob_data, abs, dyes_count).unwrap_or_default()
    } else {
        Vec::new()
    };
