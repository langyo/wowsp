...
function switchLocale(code: string) {
  locale.value = code;
  try { localStorage.setItem("wowsp-site-locale", code); } catch {}
}
...
<template>
  <div class="min-h-screen flex flex-col items-center justify-center px-6 py-16">
    ...
    <div class="flex flex-wrap justify-center gap-2 mb-12 max-w-xl">
      <button
        v-for="l in locales"
        :key="l.code"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl w-full mb-12">
      <div v-for="k in featureKeys" :key="k" class="bg-[#131e32] border border-[#1e3250] rounded-xl p-5">
        <h3 class="text-sm font-semibold mb-2 text-[#7aa2f7]">{{ t(`features.${k}.title`) }}</h3>
        <p class="text-sm leading-relaxed text-[#a0b8d4]">{{ t(`features.${k}.desc`) }}</p>
      </div>
    </div>
